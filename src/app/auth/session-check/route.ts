import { type NextRequest, NextResponse } from "next/server";

import { createClient } from "@/lib/supabase/server";
import { getCurrentUserWithDiagnostic } from "@/server/auth/get-current-user";

// Temporary, privacy-safe production diagnostic for the OTP redirect loop.
// Never return cookie values, tokens, user IDs, or email addresses.
export async function GET(request: NextRequest) {
  const authCookies = request.cookies.getAll().filter(({ name }) =>
    /^sb-.+-auth-token(?:\.\d+)?$/.test(name),
  );
  const hasAuthCookie = authCookies.length > 0;

  if (!hasAuthCookie) {
    return diagnosticResponse({ hasAuthCookie, cookieParts: 0, claims: "missing_cookie", user: "not_checked" });
  }

  const claims = (await getCurrentUserWithDiagnostic()).reason;
  let user: "valid" | "invalid" | "request_failed" = "invalid";
  let userError: { name?: string; code?: string; status?: number } | undefined;
  try {
    const supabase = await createClient();
    const result = await supabase.auth.getUser();
    user = result.data.user ? "valid" : result.error ? "request_failed" : "invalid";
    if (result.error) {
      userError = {
        name: result.error.name,
        code: result.error.code,
        status: result.error.status,
      };
    }
  } catch {
    user = "request_failed";
    userError = { name: "UnexpectedError" };
  }

  return diagnosticResponse({ hasAuthCookie, cookieParts: authCookies.length, claims, user, userError });
}

function diagnosticResponse(result: {
  hasAuthCookie: boolean;
  cookieParts: number;
  claims: string;
  user: string;
  userError?: { name?: string; code?: string; status?: number };
}) {
  const response = NextResponse.json(result);
  response.headers.set("Cache-Control", "no-store");
  return response;
}
