...
export async function POST(
  ...
  try {
    ...
    if (error instanceof ZodError) {
      return routineErrorResponse(400, "VALIDATION_ERROR", "完成记录不符合要求。", error.flatten().fieldErrors);
    }
    if (error instanceof UsageRoutineNotFoundError) {
      return routineErrorResponse(404, "ROUTINE_NOT_FOUND", "找不到当前用户的方案。");
    }
    return routineErrorResponse(500, "USAGE_HISTORY_WRITE_FAILED", "无法保存完成记录。");
  }
}
