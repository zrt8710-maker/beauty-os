...
describe("Volcengine Skin Conversation provider", () => {
  ...
  it("sends a fresh no-tools stateless structured-output request", async () => {
    ...
    await provider.extract({ message: "今天长痘了。", active_turn_context: [], recorded_date: "2026-08-29", existing_checkin: null, profile_context: null });
  ...
  it("passes only the approved long-term profile background to Doubao", async () => {
    ...
    await provider.extract({ message: "鼻子下午比平时油很多", active_turn_context: [], recorded_date: "2026-08-29", existing_checkin: null, profile_context: { skin_type: "oily", sensitivity_level: 2, skin_goals: ["oil_control"] } });
  ...
  it("passes all same-turn concerns without forcing a next concern", async () => {
    const fetchImpl = vi.fn().mockResolvedValue(new Response(JSON.stringify({ output_text: JSON.stringify(output) }), { status: 200 }));
    const provider = createVolcengineSkinConversationProvider({ apiKey: "test-key", model: "test", baseUrl: "https://example.test/responses", fetchImpl });
    await provider.extract({ message: "T区有点油，脸颊有点干，鼻子还有黑头", active_turn_context: [], recorded_date: "2026-09-01", existing_checkin: null, profile_context: { skin_type: "combination", sensitivity_level: 1, skin_goals: [], long_term_skin_baseline: { usual_oily_areas: ["t_zone"], usual_dry_areas: [], recurring_tendencies: [] } } });
    const body = JSON.parse(String((fetchImpl.mock.calls[0] as [string, RequestInit])[1].body));
    const modelInput = JSON.parse(body.input[0].content[0].text);
  ...
  it("rejects provider errors but accepts a reply-only conversational response", async () => {
    ...
    await expect(unavailable.extract({ message: "今天长痘了。", active_turn_context: [], recorded_date: "2026-08-29", existing_checkin: null, profile_context: null })).rejects.toThrow("503");
    ...
    await expect(malformed.extract({ message: "今天长痘了。", active_turn_context: [], recorded_date: "2026-08-29", existing_checkin: null, profile_context: null })).resolves.toMatchObject({ reply: "x", assessments: [] });
  });
  ...
  it("keeps a completed turn usable when optional structured JSON is truncated after a valid reply", async () => {
    ...
    await expect(provider.extract({ message: "没有了", active_turn_context: ["Beauty OS：还有其他今天想记下的吗？没有的话我就按这些整理。"], recorded_date: "2026-09-01", existing_checkin: null, profile_context: null })).resolves.toMatchObject({ reply: "好，我按这些整理。", assessments: [] });
  });
  ...
  it("preserves a valid reply when raw optional structured fields are incompatible", async () => {
    ...
    const result = await provider.extract({ message: "脸颊有点干", active_turn_context: [], recorded_date: "2026-08-30", existing_checkin: null, profile_context: null });
  ...
  it("accepts the Volcengine Responses output content shape", async () => {
    ...
    await expect(provider.extract({ message: "今天长痘了。", active_turn_context: [], recorded_date: "2026-08-29", existing_checkin: null, profile_context: null })).resolves.toMatchObject({ readiness: "continue" });
  });
  ...
  it("normalizes any provisional daily state from a continuing response", async () => {
    ...
    await expect(provider.extract({ message: "额头有很多闭口", active_turn_context: [], recorded_date: "2026-08-30", existing_checkin: null, profile_context: null })).resolves.toMatchObject({ readiness: "continue", daily_state: null });
  });
