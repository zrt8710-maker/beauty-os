...
describe("OpenAI Skin Conversation provider", () => {
  it("uses natural text for an ordinary turn and reserves structure for finalization", async () => {
    const fetchImpl = vi.fn().mockResolvedValue(new Response(JSON.stringify({ output_text: "明白，是整张脸都有一点油。" }), { status: 200 }));
    const provider = createOpenAiSkinConversationProvider({ apiKey: "test-key", fetchImpl });
    const result = await provider.extract({ message: "是整张脸都有点油", active_turn_context: ["Beauty OS：是 T 区还是整张脸？"], recorded_date: "2026-09-23", existing_checkin: null, profile_context: null });
    ...
    expect(JSON.parse(body.input[0].content[0].text).active_turn_context).toHaveLength(1);
    expect(result).toMatchObject({ reply: "明白，是整张脸都有一点油。", assessments: [] });
  });
  ...
  it("uses a stateless strict structured-output Responses request", async () => {
    ...
    await provider.extract({ finalize_requested: true, message: "脸不舒服", active_turn_context: [], recorded_date: "2026-08-29", existing_checkin: null, profile_context: null });
  ...
  it("accepts a reply-only provider response", async () => {
    ...
    const provider = createOpenAiSkinConversationProvider({ apiKey: "test-key", fetchImpl });
    await expect(provider.extract({ finalize_requested: true, message: "脸不舒服", active_turn_context: [], recorded_date: "2026-08-29", existing_checkin: null, profile_context: null })).resolves.toMatchObject({ reply: "x", assessments: [] });
  });
  ...
  it("preserves a valid reply when optional structured JSON is truncated", async () => {
    ...
    const provider = createOpenAiSkinConversationProvider({ apiKey: "test-key", fetchImpl });
    await expect(provider.extract({ finalize_requested: true, message: "没有了", active_turn_context: [], recorded_date: "2026-09-01", existing_checkin: null, profile_context: null })).resolves.toMatchObject({ reply: "好，我按这些整理。", assessments: [] });
  });
});
