...
-- Keep the existing RPC contracts and explicitly resolve calculations as integer
-- before assigning the bounded value back to smallint variables.
...
begin
  ...
  if p_match_source = 'barcode_exact' then
    ...
    resolved_confidence := least(
      90,
      candidate_record.confidence::integer
    )::smallint;
