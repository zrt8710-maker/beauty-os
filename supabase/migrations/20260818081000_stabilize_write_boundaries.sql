-- Stabilization only: tighten write boundaries without adding business tables.

-- Usage history remains readable through RLS, but all writes must pass through
-- record_routine_usage(), which already validates the routine, products, and payload.
revoke insert on table public.usage_history from authenticated;
revoke insert on table public.usage_history_products from authenticated;

alter function public.record_routine_usage(
  uuid,
  date,
  text,
  text,
...
-- Product draft candidates are derived data. Users may edit only manual draft
-- fields; deterministic candidate fields are persisted through the RPC below.
revoke insert, update on table public.product_drafts from authenticated;
...
grant insert (
  ...
  created_at,
  updated_at
) on table public.product_drafts to authenticated;
...
create function public.normalize_product_match_text(value text)
returns text
language sql
immutable
strict
...
as $$
  select pg_catalog.lower(
    pg_catalog.regexp_replace(value, '[^[:alnum:]]+', '', 'g')
  );
$$;

revoke all on function public.normalize_product_match_text(text) from public, anon, authenticated;

create function public.save_product_draft_match(
  p_draft_id uuid,
...
begin
  ...
  if p_match_source = 'barcode_exact' then
    ...
    resolved_confidence := pg_catalog.least(90, candidate_record.confidence);
...
revoke all on function public.save_product_draft_match(uuid, uuid, text) from public, anon;
grant execute on function public.save_product_draft_match(uuid, uuid, text) to authenticated;

-- Purchase analysis rows are immutable derived records. Direct INSERT is removed;
-- the persistence RPC binds ownership to auth.uid() and verifies deterministic
-- final-score and decision invariants before storing a service-calculated result.
revoke insert on table public.purchase_analyses from authenticated;

create function public.persist_purchase_analysis(
...
revoke all on function public.persist_purchase_analysis(
  uuid,
  jsonb,
  jsonb,
  jsonb,
...
grant execute on function public.persist_purchase_analysis(
  uuid,
  jsonb,
  jsonb,
  jsonb,
