-- Confirming a draft is the sole controlled transition to `confirmed`.
-- Direct product_drafts UPDATE remains unavailable to authenticated users.

create or replace function public.confirm_product_draft(
  p_draft_id uuid,
...
begin
  ...
  if draft_record.candidate_catalog_product_id is not null
    and not exists (
      select 1
      from public.catalog_products
      where id = draft_record.candidate_catalog_product_id
        and status = 'verified'
    ) then
    raise exception 'DRAFT_CANDIDATE_NOT_VERIFIED' using errcode = '23514';
  end if;

  if p_catalog_product_id is not null
    and draft_record.candidate_catalog_product_id is distinct from p_catalog_product_id then
    raise exception 'INVALID_CATALOG_CONFIRMATION' using errcode = '23514';
  end if;
  ...
  set
    status = 'confirmed',
    knowledge_confirmed_at = case
      when p_catalog_product_id is null then null
      else now()
    end,
    updated_at = now()
