-- The original function's local product_id variable conflicted with
-- user_owned_products.product_id during routine ownership validation.
-- Replace only the function body; existing feedback data remains untouched.

create or replace function public.record_usage_feedback_message(
  p_routine_id uuid,
...
declare
  ...
  item jsonb;
  operation text;
  feedback_owned_product_id uuid;
begin
  ...
  loop
    feedback_owned_product_id := (item->>'owned_product_id')::uuid;
    if not exists (
      select 1
      ...
      where routine_steps.routine_id = p_routine_id
        and routine_steps.owned_product_id = feedback_owned_product_id
    ...
    if operation = 'retract' then
      ...
      where usage_history_id = usage_record.id
        and owned_product_id = feedback_owned_product_id;
      ...
      insert into public.usage_history_products (
        ...
        usage_record.id, feedback_owned_product_id, (item->>'rating')::smallint,
