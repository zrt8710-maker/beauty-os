-- Candidate Catalog identities intentionally have no canonical classification until review.
-- Use the confirmed user asset classification only for fields still unknown; verified
-- Catalog classifications remain authoritative. The delegated idempotent RPC is unchanged.
...
begin
  ...
  if p_resolution_kind = 'catalog' then
    ...
    insert into public.products (
      ...
      coalesce(catalog_record.category, p_category),
      coalesce(catalog_record.subcategory, p_subcategory),
      coalesce(catalog_record.product_type, p_product_type),
      catalog_record.id,
